...
Mark encouraged everyone in Tesla digital products team to move into
the new CI/CD pipeline. There is NO manual release in his notebook. He
created a POC project on CI/CD and demo'd to the entire team to show how
to move away from manual release. Successfully, he automated whole process
for referral projects + many more. He implemented the Atm pipeline
and documented the whole pipeline with every single configuration and
its explanation with example for all different languages.

-- Vijayalaxmi Tiripudur, Staff Software Engineer at Tesla
